    ...
    with sync_playwright() as p:
        ...
        if submit_btn:
            ...
            try:
                ...
                # Check for "All" filter link and click it if possible to ensure all data is shown
                # The output showed '...All' in the first header.
                
                # Let's try to find the "All" link.
                try:
                    all_link = page.get_by_text("All", exact=True)
                    if all_link.is_visible():
                        print("Clicking 'All' filter...")
                        all_link.click()
                        page.wait_for_timeout(2000) # Wait for reload
                except:
                    print("'All' link not found or not clickable.")

                # Re-query rows after potential reload
                rows = page.query_selector_all("tbody tr")
                print(f"Found {len(rows)} rows of data after checking 'All'.")

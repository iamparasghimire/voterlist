...
def scrape_polling_centre(page, mun_name, ward_name, center_name):
    ...
    while True:
        # Scrape rows using evaluate for stability and speed
        # Extract all cell texts for all rows in the current tbody
        ...
        print(f"  Page {page_num}: Found {len(page_data)} rows.")
        ...
        # The class 'paginate_enabled_next' indicates it is enabled.
        # We can check this in the page context as well.
        has_next = page.evaluate("""() => {
            const nextBtn = document.querySelector('#tbl_data_next');
            return nextBtn && nextBtn.classList.contains('paginate_enabled_next');
        }""")
        
        if has_next:
            # Click next
            page.click("#tbl_data_next")
            page_num += 1
            
            # Wait for table update
            # We can wait for the processing indicator to disappear if there is one, 
            # or wait for the table to be repainted.
            # A simple timeout is often safest if we don't know the exact event.

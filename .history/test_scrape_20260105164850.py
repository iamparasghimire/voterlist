        ...
        if submit_btn:
            ...
            try:
                ...
                for i, sel in enumerate(selects):
                    print(f"Select #{i}: name='{sel.get_attribute('name')}', id='{sel.get_attribute('id')}'")
                    # Print options
                    opts = sel.query_selector_all("option")
                    print(f"  Options: {[o.inner_text() for o in opts]}")
                    
                    # If it looks like a length menu (10, 25, 50, 100), select the largest
                    if any("100" in o.inner_text() for o in opts):
                        print("Found length menu! Selecting 100 (or max)...")
                        # Try to select the largest value
                        vals = [o.get_attribute("value") for o in opts]
                        # Assuming values are numbers, pick max
                        try:
                            max_val = max(vals, key=lambda x: int(x) if x.isdigit() else 0)
                            print(f"Selecting max value: {max_val}")
                            sel.select_option(value=max_val)
                            page.wait_for_timeout(3000)
                        except:
                            print("Could not determine max value.")
                ...
                if next_btn:
                    ...
                    # Check if it's disabled. Often it has class 'disabled' or parent has class 'disabled'
                    # Let's print the element and its parent
                    print(f"Next button HTML: {next_btn.evaluate('el => el.outerHTML')}")
                    parent = next_btn.evaluate_handle('el => el.parentElement')
                    print(f"Next button parent HTML: {parent.evaluate('el => el.outerHTML')}")
                else:
                    print("'Next' button not found.")
                    # Look for any pagination container
                    pager = page.query_selector("div.dataTables_paginate")
                    if pager:
                        print(f"Found DataTables pagination: {pager.inner_html()}")
